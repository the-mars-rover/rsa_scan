...
class RsaDriversScanner extends StatefulWidget {
  final Widget overlay;

  const RsaDriversScanner({Key key, this.overlay}) : super(key: key);

  @override
  _RsaDriversScannerState createState() => _RsaDriversScannerState();
}

/// The state for the [RsaDriversScanner] widget.
class _RsaDriversScannerState extends State<RsaDriversScanner>
  ...
  RsaDriversLicense _scannedDocument;
  ...
  Future<void> _scanImageFrame(CameraImage availableImage) async {
    ...
    _scannerBusy = true;
    final scannedDrivers = await ScannerUtils.scanDrivers(availableImage);
    if (scannedDrivers != null) {
      setState(() {
        this._scannedDocument = scannedDrivers;
      });
